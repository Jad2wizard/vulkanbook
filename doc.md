...
6. 最后在录制渲染命令时需要录制将描述符集实例绑定到管线的绑定点上。包括pipelineBindingPoint参数，指定是渲染管线还是计算管线，layout为pipelineLayout的句柄，firstSet为第一个描述符集的索引（一般为0，这个应该和shader代码中layout(set = X)的X有关，其中X=firstset + descriptorSets中描述符集的索引）、descriptorSets为描述符集buffer的指针等参数。虽然在创建描述符集时已经传入了描述符布局，但在渲染时还是需要将描述符集显示地绑定到管线上。